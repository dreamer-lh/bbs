...
$pageStr = '';
//拼接首页
$pageStr .= "<a href='./list_father.php?pageNum=1'>首页</a>";
//拼接上一页
//上一页
$prePage = $pageNum - 1 > 0 ? $pageNum - 1 : 1;
$pageStr .= "<a href='./list_father.php?pageNum=$prePage'><<上一页</a>";
//拼接中间页
//定义循环开始页和结束页
/*if ($pageCount <= 5) {
	$pageStart = 1;
...
for ($i=$pageStart; $i <= $pageLast; $i++) { 
	if ($i==$pageNum) {
		...
		$pageStr .= "<a href='./list_father.php?pageNum=$i'>$i</a>";
	}
}
//拼接下一页
//下一页
$nextPage = $pageNum + 1 < $pageCount ? $pageNum + 1 : $pageCount;
$pageStr .= "<a href='./list_father.php?pageNum=$nextPage'>下一页>></a>";
//拼接尾页
$pageStr .= "<a href='./list_father.php?pageNum=$pageCount'>尾页</a>";

//统计查询帖子表
$start = $pageRepNum*($pageNum-1);
$offset = $pageRepNum;
$page_sql = "select * from publish left join user on pub_owner=user_name order by pub_time desc limit $start,$offset";
